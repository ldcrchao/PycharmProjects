...
def plt_learning_curve(estimator,title,X,y,ylim=None,cv=None,train_size=np.linspace(.1,1.0,5)):
    plt.figure()
    plt.title(title)
    if ylim is not None:
        plt.ylim(*ylim)
    ...
    # 测试集/训练集 主曲线
    plt.plot(train_sizes,train_scores_mean,"o-",color="r",label="交叉验证-训练集得分") # 图例可以使用label控制
    plt.plot(train_sizes,test_scores_mean,"o-",color="g",label="交叉验证-测试集得分")
    plt.legend(loc="best")
    plt.axis('tight')
    plt.show()
    return plt
def EvaluationIndex(X_test,y_test,y_pre,clf,confusion_matrix=True):
    ...
    print('平均准确率ACU为:',ACU)
    # 2、精准率 PRECISON
    Precison_Score = metrics.precision_score(y_test,y_pre ,average='weighted') # 精准率
    ...
    F_beta = metrics.fbeta_score(y_test,y_pre ,beta=0.5,average='weighted') # beta 确定组合分数中的召回权重
    ...
    Evaluation_index.columns = ['平均准确率ACU', '精准率Precision', '召回率Recall', 'F1得分', '方均根误差RMS', '汉明损失HLoss',
                                'Matthews相关系数', 'R2系数', '平衡精度BAS', '加权谐波平均值F_beta', '相似系数Jaccard',
                                '调整互信息AMI', '标准化互信息NMI', '方差回归得分EVS', 'Fowlkes-Mallows指数FMI']
...
EIK  = pd.DataFrame()
for i in range(1000) :
  X_train, X_test, y_train, y_test = train_test_split(X,y,test_size=0.2) # 训练数据比例8成
  clf=svm.SVC(kernel='rbf',C=1,probability=True)
  clf.fit(X_train ,y_train ) # 训练模型
  y_pre = clf.predict(X_test)
  Evaluation_index = EvaluationIndex(X_test,y_test,y_pre,clf,confusion_matrix=False)
...
ax1.text((min(ACU)+1.0)/2 - 0.1,meanACU+0.02,"Avarage ACU : "+str(round(meanACU,5)),fontsize=14,
         verticalalignment='center',horizontalalignment='center',family='Times New Roman')
ax1.axis('tight')
ax1.legend(('折线图','平均值'),loc='center left')
plt.fill_betweenx(np.arange(min(ACU)-0.05,1.05,0.05),1.0,1.05,alpha=0.2,color="r",lw=2)
ax1.set_xlim(min(ACU),1.05)
ax1.set_ylim(min(ACU),1.0)
ax1.text(1.025,(min(ACU)+1)/2 ,"区域不存在" ,fontsize=10.5,verticalalignment='center',horizontalalignment='center',family='SimHei')
plt.title('RBF核函数留出法预测准确率分布图(训练次数:1000次)')
fig.tight_layout()
plt.show()
#%%
'''K折交叉验证'''
...
for train_index, test_index in kf.split(X):
    ...
    Evaluation_indexkfold = EvaluationIndex(test_X ,test_y,y_pre1,clf1,confusion_matrix=False)
...
for C in C_s:
    clf.C=C
    this_scores=cross_val_score(clf,X,y,cv=n_folds,n_jobs=1) # n_jobs用于进行计算的CPU数量
    scores.append(np.mean(this_scores))
    scores_std.append(np.std(this_scores))
ax1 , = plt.semilogx(C_s,scores) # 绘制得分
ax2 , = plt.semilogx(C_s,np.array(scores)+np.array(scores_std),'r--') # 绘制得分的上包络线
ax3 , = plt.semilogx(C_s,np.array(scores)-np.array(scores_std),'b--') # 绘制得分的下包络线
plt.fill_between(C_s,np.array(scores)+np.array(scores_std),np.array(scores)-np.array(scores_std),alpha=0.1,color="r") # 着色
...
plt.ylabel('交叉验证得分')
plt.xlabel("正则化系数C")
plt.title(f"RBF核函数不同正则化系数的{n_folds}折交叉验证得分趋势图")
plt.axis('tight')
plt.show()
#%%
'''不同训练集个数的学习率曲线'''
...
title = "RBF核函数不同训练集比例的10折学习率曲线(固定正则化系数C=1)"
estimator = svm.SVC(kernel='rbf', probability=True)
plt_learning_curve(estimator,title,X,y,ylim=(0.7,1.0),cv=cv,train_size=train_size)
#%%
'''模型评估 : 不同gamma的模型验证曲线'''
param_range=np.logspace(-30,30,100)
train_scores,test_scores=validation_curve(svm.SVC(kernel='rbf'),X,y,param_name="gamma",param_range=param_range,cv=10,scoring="accuracy",n_jobs=1)
train_scores_mean=np.mean(train_scores,axis=1) # 训练集得分
train_scores_std=np.std(train_scores,axis=1)
test_scores_mean=np.mean(test_scores,axis=1) # 测试集得分
test_scores_std=np.std(test_scores,axis=1)
plt.title("RBF核函数不同gamma系数的交叉验证得分趋势图")
plt.xlabel("内核系数$\gamma$")
plt.ylabel("得分")
lw=1.5
# 训练集 主曲线
ax1 ,= plt.semilogx(param_range,train_scores_mean,label="交叉验证-训练集得分",color="r",lw=lw)
ax2 , = plt.semilogx(param_range,np.array(train_scores_mean)+np.array(train_scores_std),'b--',label='训练集上包络线',lw=0.5) # 训练集上包络线
ax3 , = plt.semilogx(param_range,np.array(train_scores_mean)-np.array(train_scores_std),'b--',label='训练集下包络线',lw=0.5) # 训练集下包络线
# 训练集 上下包络线
plt.fill_between(param_range,train_scores_mean-train_scores_std,train_scores_mean+train_scores_std,alpha=0.2,color="r",lw=lw)
# 测试集 主曲线
ax4 ,= plt.semilogx(param_range,test_scores_mean,label="交叉验证-测试集得分",color="g",lw=lw)
ax5 , = plt.semilogx(param_range,np.array(test_scores_mean)+np.array(test_scores_std),'y--',label='测试集上包络线',lw=0.5) # 测试集上包络线
ax6 , = plt.semilogx(param_range,np.array(test_scores_mean)-np.array(test_scores_std),'y--',label='测试集下包络线',lw=0.5) # 测试集下包络线
# 测试集 上下包络线
plt.fill_between(param_range,test_scores_mean-test_scores_std,test_scores_mean+test_scores_std,alpha=0.2,color="g",lw=lw)
minscore = min( np.min(np.array(train_scores_mean)-np.array(train_scores_std)),
                np.min(np.array(test_scores_mean)-np.array(test_scores_std)) )
plt.ylim((minscore,1))
plt.legend( loc='best')
# 不显示指数负号解决方案
ax = plt.gca()
...
clf=svm.SVC(kernel='rbf',C=1,probability=True)
score , permutation_scores , pvalues = permutation_test_score (clf,X,y,scoring='accuracy',cv=10,n_jobs= 1,n_permutations= 100)
permutation_score = pd.Series(permutation_scores)
fig, ax = plt.subplots()
ax.grid(axis='y')
permutation_score.plot.kde(ax=ax, linewidth=2.5,bw_method=0.15)
permutation_score.plot.hist(density=True,color='c', ax=ax)
plt.ylim((0,len(permutation_score)))
plt.title('RBF核函数排列分数直方图和核密度估计图')
plt.xlabel('排列分数')
plt.ylabel('频数')
labels = ["核密度估计图","直方图"]
plt.legend(labels)
...
plt.fill_between(np.linspace(0,len(ACU),len(ACU)),meanACU,ACU[-1],alpha=0.1,color='r')
plt.text(  (0+len(ACU))/2,meanACU+0.008,"Avarage ACU : "+str(round(meanACU,5)),
         verticalalignment='center',horizontalalignment='center',family ='Times New Roman',fontsize=14 )
plt.text(  (0+len(ACU))/2,ACU[-1]-0.008,"True ACU : "+str(round(ACU[-1],5)),
         verticalalignment='center',horizontalalignment='center',family ='Times New Roman',fontsize=14 )
plt.xlabel('训练次数')
plt.ylabel('准确率')
plt.xlim((0,len(ACU)))
...
for train_index, test_index in lpo.split(X):
    ...
    acu = SumACU / K  # 当前的准确率 例如当前训练次数10次,可能全部正确的2个,2/3正确的3个,1/3正确的1个,其它不正确,那么总的准确率为[2×1+3×(2/3)+1×(1/3)]/10=13/30
    ACU.append(acu)
endtime = time()
...
plt.text(  (0+len(ACU))/2,ACU[-1]-0.008,"True ACU : "+str(round(ACU[-1],5)),
         verticalalignment='center',horizontalalignment='center',family ='Times New Roman',fontsize=14 )
plt.text(  (0+len(ACU))/2,min(ACU)+0.12,"Training Time : "+str(round(Time,3))+' s',
         verticalalignment='center',horizontalalignment='center',family ='Times New Roman',fontsize=14 )
plt.xlabel('训练次数')
plt.ylabel('准确率')
plt.xlim((0,len(ACU)))
...
plt.tight_layout()
plt.legend()
plt.title(f'RBF核函数留{p}法准确率变化曲线')
plt.show()
...
np.random.set_state(state) # 保证样本和标签以相同的规律被打乱
np.random.shuffle(y)
kf = RepeatedKFold(n_splits=10, n_repeats=10, random_state=0) # random_state=0可以保证每次K折交叉验证的随机打乱规律不同
EIK = pd.DataFrame()
P = 0
for train_index, test_index in kf.split(X):
    idx = kf.get_n_splits()
    ...
    Evaluation_indexkfold = EvaluationIndex(test_X ,test_y,y_pre2,clf2,confusion_matrix=False)
    EIK = pd.concat([EIK,Evaluation_indexkfold],axis=0,ignore_index= True )
#%%
'''蒙特卡洛近似和奈斯特罗姆近似'''
X = pd.read_csv("C:/Users\chenbei\Desktop\陈北个人论文\图源数据及其文件/Circuit_Breaker_SecondLevelLabel.csv",encoding='gbk')
...
fourier_approx_svm_rbf = pipeline.Pipeline([("feature_map", feature_map_fourier), ("svm", svm.SVC(gamma=.2))]) # 基于RBF构建第一个管道
nystroem_approx_svm_rbf = pipeline.Pipeline([("feature_map", feature_map_nystroem),("svm", svm.SVC(gamma=.2))]) # 基于RBF构建第二个管道
