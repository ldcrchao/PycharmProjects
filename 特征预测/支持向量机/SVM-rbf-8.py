...
def plt_learning_curve(estimator,title,X,y,ylim=None,cv=None,train_size=np.linspace(.1,1.0,5)):
    ...
    plt.show()
    return plt
def EvaluationIndex(X_test,y_test,y_pre,clf,confusion_matrix=True,title=None):
    '''评价指标'''
    # 1、准确度 ACU
    ...
    if confusion_matrix :
       ...
       plt.title(f'{title}的混淆矩阵(10折交叉验证)',fontproperties=font_set)
       plt.show()
    # 8、方均根误差
...
starttime = time()
for i in range(100) :
  X_train, X_test, y_train, y_test = train_test_split(X,y,test_size=0.3) # 训练数据比例8成
...
ax1.text((min(ACU)+1.0)/2 ,meanACU+0.005,"平均准确率 : "+str(round(meanACU,5)),fontsize=14,
         verticalalignment='center',horizontalalignment='center')
ax1.text( (min(ACU)+1.0)/2,min(ACU)+0.006,"训练时间 : "+str(round(Time,5))+' s',
...
ax1.text((1.0+1.02)/2,(min(ACU)+1)/2 ,"区域不存在" ,fontsize=10.5,verticalalignment='center',horizontalalignment='center',family='SimHei')
plt.title('径向基核RBF留出法预测准确率分布图(训练次数:100次)')
fig.tight_layout()
plt.show()
#%%
'''K折交叉验证'''
...
plt.ylabel('交叉验证得分')
plt.xlabel("正则化系数C")
plt.title(f"径向基核RBF不同正则化系数的{n_folds}折交叉验证得分趋势图")
plt.axis('tight')
plt.show()
#%%
'''不同训练集个数的学习率曲线'''
...
title = "径向基核RBF不同训练集比例的10折交叉验证得分(固定正则化系数C=0.1)"
...
plt.title("径向基核RBF不同$\gamma$系数的交叉验证得分趋势图")
...
ax1.fill_betweenx(np.arange(min(ACU)-0.05,1.02,0.05),1.0,1.02,alpha=0.2,color="r",lw=2)
ax1.text(  (min(ACU)+1.0)/2 ,meanACU+0.005,"平均准确率 : "+str(round(meanACU,5)),
         verticalalignment='center',horizontalalignment='center',fontsize=14 )
ax1.text(  (min(ACU)+1.0)/2 ,ACU[-1]+0.005,"真实准确率 : "+str(round(ACU[-1],5)),
         verticalalignment='center',horizontalalignment='center',fontsize=14 )
ax1.text((1.0+1.02)/2,(min(ACU)+1)/2 ,"区域不存在" ,fontsize=10.5,family='SimHei',verticalalignment='center',horizontalalignment='center')
ax1.text( (min(ACU)+1.0)/2,min(ACU)+0.004,"训练时间 : "+str(round(Time,5))+' s',
         verticalalignment='center',horizontalalignment='center',fontsize=14 )
ax1.axis('tight')
ax1.legend(('折线图','平均值'),loc='center left')
ax1.set_xlim(min(ACU),1.02)
ax1.set_ylim(min(ACU),1.0)
fig.tight_layout()
plt.title(f'径向基核RBF留1法准确率变化曲线(训练次数:{len(ACU)}次)')
plt.show()
#%%
'''留P法'''
p = 2
...
ax1.text(  (min(ACU)+1.0)/2 ,ACU[-1]+0.002,"真实准确率 : "+str(round(ACU[-1],5)),
         verticalalignment='center',horizontalalignment='center',fontsize=14 )
ax1.text((1.0+1.02)/2,(min(ACU)+1)/2 ,"区域不存在" ,fontsize=10.5,family='SimHei',verticalalignment='center',horizontalalignment='center')
ax1.text( (min(ACU)+1.0)/2,min(ACU)+0.02,"训练时间 : "+str(round(Time,5))+' s',
         verticalalignment='center',horizontalalignment='center',fontsize=14 )
ax1.axis('tight')
ax1.legend(('折线图','平均值'),loc='center left')
ax1.set_xlim(min(ACU),1.02)
ax1.set_ylim(min(ACU),1.0)
fig.tight_layout()
plt.title(f'径向基核RBF留2法准确率变化曲线(训练次数:{len(ACU)}次)')
plt.show()
#%%
'''P次K折交叉验证'''
state = np.random.get_state() # 必须先打乱原来的数据集合标签集,否则初始的有序状态会导致交叉验证预测率很低
...
for train_index, test_index in kf.split(X):
    ...
    Evaluation_indexkfold = EvaluationIndex(test_X ,test_y,y_pre2,clf2,confusion_matrix=False,title='径向基核RBF')
    EIK = pd.concat([EIK,Evaluation_indexkfold],axis=0,ignore_index= True )
