...
class MyTreeWidget(QMainWindow):
    def __init__(self,parent=None):
        ...
        self.ui.treeWidget.header().setDefaultSectionSize(650) # 设置两列之间的距离
        self.ui.treeWidget.header().setMinimumSectionSize(650)


        self.ui.dockWidget.setFeatures(QDockWidget.AllDockWidgetFeatures) # 停靠区的属性,包括停靠区可关闭、停靠区可移动、停靠区可浮动、在停靠区左侧显示垂直标题栏
        self.ui.dockWidget.setAllowedAreas(Qt.LeftDockWidgetArea | Qt.RightDockWidgetArea) # 允许停靠区域 , 左右浮动
    ...
    def _intTree(self):
        ...
        if paritem == None :
           self.ui.action_AddFile.setEnabled(False) # 如果没有父节点或者没有选中父节点 不可以添加文件和删除节点
        ...
        item = QTreeWidgetItem(TreeWidgetItem.itTopItem.value)
        ...
        item.setText(TreeColNum.colItem.value,"女朋友照片")
        ...
        item.setTextAlignment(TreeColNum.colItemType.value,Qt.AlignVCenter | Qt.AlignHCenter)
    ...
    def on_action_AddFile_triggered(self):
        # 设置文件扩展名过滤,用双分号间隔。eg : "All Files (*);;PDF Files (*.pdf);;Text Files (*.txt)
        ...
        for i in range (len(filelist)) :
            ...
            #print(filelist[i] ) # C:/Users/chenbei/PycharmProjects/python学习工程文件夹/GUI/icons/165.JPG
            fileinfo = QFileInfo(fullFilename) # 在python当中保存的路径
            #print(fileinfo) # <PyQt5.QtCore.QFileInfo object at 0x000001BDBD014518>
    ...
    def _displayImage(self,item):
        filename = item.data(TreeColNum.colItem.value,Qt.UserRole) # 通过setData设置的个人用户数据可通过data取出
