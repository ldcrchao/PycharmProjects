...
clf = svm.LinearSVC ()#线性SVM分类器
clf.fit(iris.data,iris.target) #训练
print(clf.predict([[6,3,5,1.8]])) #预测结果
clf.coef_ # 训练好的参数
#%%
from Car import Car  # package命名Car，从该包调用Car.py模块
mycar = Car.electric_car(brand='tesila',type= 'TEX-6',year= 2021,mileage=100,gastank= 9)
mycar.info()
#%%
with open('C:/Users\chenbei\PycharmProjects\python学习工程文件夹\杂类\pi.txt') as file_object:
    #contents = file_object.read()
    #print(contents)
    lines = file_object.readlines() #存储为列表类型
for line in lines : #逐行读取
        print(line.strip()) #文本文件有换行符，print也有换行符，rstrip可以消除多余空白行,strip可以消除空格+换行符
string1 = ''
for line in lines:
    string1 += line.strip() #去除换行符和空格
print(float(string1)) #转换成浮点数形式
'''
...
def ADset(tcpClient):
    ...
    ad_frequency = [0x90,0xd0,0x03,0x00] #ad采样频率 0x7d00=32000 007d 0000
...
def ADDataRead(tcpClient):
    ...
    nop = [0x00, 0x00]  # 备用
    ulLength = [0x98, 0x3a, 0x00, 0x00]  # 指定读取采集数据的个数  0—2^32 ,但是每次最多发送720个数据 02d0 = 720 ，这里指定接受 120000个数 c0d4 0100 <-> 0001 d4c0
    ...
    tcpClient.send(read_message)
    # 接收返回结果
    read_response = tcpClient.recv(1448*25)
    # 解析返回结果
    read_result = struct.unpack("%dB" % (len(read_response)), read_response)
    # 删除头8个字节
...
if __name__ == '__main__':
    ...
    while True:
        '''数据接收'''
        temp_data ,temp = ADDataRead(tcpClient) #1440个字节
        ...
        for i in h:
            channel_0.append(temp_data[i + 0])  #3780个10进制的列表
        ...
        Channel_0 = list_to_hex_to_str(channel_0)  # 7530个字符串 ,因为列表从3750~3779 一共30个是为0的
        ...
        Channel_0_10 = hex_to_signed_10_much(Channel_0) # 7530/4 =1882.5个有符号十进制数
        ...
        dict_tmp['channel_0'] = Channel_0_10
        dict_tmp['channel_1'] = Channel_1_10
        dict_tmp['channel_2'] = Channel_2_10
        dict_tmp['channel_3'] = Channel_3_10
        dict_tmp['channel_4'] = Channel_4_10
        dict_tmp['channel_5'] = Channel_5_10
        dict_tmp['channel_6'] = Channel_6_10
        dict_tmp['channel_7'] = Channel_7_10
        print(dict_tmp)
